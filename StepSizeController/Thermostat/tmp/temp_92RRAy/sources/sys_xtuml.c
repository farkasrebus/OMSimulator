...
typedef union {
  ...
  Thermostat_DomainEvents_u mc_events_in_domain_Thermostat;
...
static void ooa_loop( void )
{
  ...
  while ( true == Escher_run_flag ) {
    ...
    if ( 0 != event ) {
      ...
      if( UserBackgroundProcessingCallout() == false) {
...
{
  SimulateTime += (unsigned long) (stepSize* 1000);
  ooa_loop();
}
