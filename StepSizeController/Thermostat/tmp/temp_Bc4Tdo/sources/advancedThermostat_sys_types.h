/*----------------------------------------------------------------------------
 * File:  advancedThermostat_sys_types.h
 *
...
#ifndef ADVANCEDTHERMOSTAT_SYS_TYPES_H
#define ADVANCEDTHERMOSTAT_SYS_TYPES_H
#ifdef	__cplusplus
extern	"C"	{
#endif
...
void Escher_xtUML_run( double stepSize );
...
#define Thermostat_DOMAIN_ID 0
#define Thermostat_DOMAIN_ID_text "Thermostat"
...
#define STATE_TXN_START_TRACE( obj_kl, state_num, state_name ) do {   if(loggingSwitcher) {   XTUML_SOURCE_PROLOGUE;   printf( "Transition started:  %s State [%u] %s\n", obj_kl, state_num, state_name );} } while (0)
#endif
...
#define STATE_TXN_END_TRACE( obj_kl, state_num, state_name ) do {   if(loggingSwitcher) {   XTUML_SOURCE_PROLOGUE;   printf( "Transition complete:  %s State [%u] %s\n", obj_kl, state_num, state_name );   XTUML_TRACE_FLUSH( 0 );} } while (0)
#endif
...
#define STATE_TXN_CH_TRACE( obj_kl, state_num ) do {   if(loggingSwitcher) {   XTUML_SOURCE_PROLOGUE;   printf( "Event cannot happen:  %s current_state = %u\n", obj_kl, state_num );   XTUML_TRACE_FLUSH( 0 ); } } while (0)
#endif
...
#define XTUML_OAL_STMT_TRACE( blck_level, stmt_action ) do {   if(loggingSwitcher) {   XTUML_SOURCE_PROLOGUE;   { /* indenting */ s1_t i; for ( i = 0; i < blck_level; i++ ) printf( "  " ); }   printf( "%s\n", stmt_action );   XTUML_TRACE_FLUSH( 0 );} } while (0)
#endif
...
#endif
#endif  /* ADVANCEDTHERMOSTAT_SYS_TYPES_H */
